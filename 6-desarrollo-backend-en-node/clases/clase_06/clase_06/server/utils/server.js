...
const port = process.env.SERVER_PORT || 3_000;
  ...
  if (request.url === '/') {
    fs.readFile('public/index.html', 'utf8', (err, file) => {
      if (err) {
        ...
        response.write(JSON.stringify({ status: 404, message: 'Error al leer el index.html', err }));
        return response.end();
      }
    ...
    fs.readFile('public/assets/css/main.css', 'utf8', (err, file) => {
      if (err) {
        response.writeHead(404, { 'Content-Type': 'application/json' });
        response.write(JSON.stringify({ status: 404, message: 'Error al leer el main.css', err }));
        return response.end();
      }
    ...
    fs.readFile('public/assets/js/main.js', 'utf8', (err, file) => {
      if (err) {
        response.writeHead(404, { 'Content-Type': 'application/json' });
        response.write(JSON.stringify({ status: 404, message: 'Error al leer el main.js', err }));
        return response.end();
      }
